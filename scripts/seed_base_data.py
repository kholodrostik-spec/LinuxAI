...
def main():
    ...
    drivers = [
        ...
        ("xe", "xe", "kernel", "New Intel Xe graphics driver for newer Intel GPUs"),
    ...
    devices = [
        ...
        # Bluetooth
        ("Intel Bluetooth", "Intel", "bluetooth", None, None, "Bluetooth often paired with Intel Wi-Fi adapters"),
        ("Realtek Bluetooth", "Realtek", "bluetooth", None, None, "Bluetooth often paired with Realtek Wi-Fi adapters"),
        ("MediaTek Bluetooth", "MediaTek", "bluetooth", None, None, "Bluetooth often paired with MediaTek Wi-Fi adapters"),
        ("Qualcomm Bluetooth", "Qualcomm", "bluetooth", None, None, "Bluetooth often paired with Qualcomm Wi-Fi adapters"),
        ...
        ("Intel RST Controller", "Intel", "storage", None, None, "May require switching BIOS storage mode from RAID/RST to AHCI"),
    ]
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            apt_like_ids = [ubuntu_id, debian_id, popos_id, mint_id]
            ...
            for distro_id in [ubuntu_id, popos_id, mint_id]:
                packages.extend([
                    ("linux-firmware", distro_id, "Firmware package for Wi-Fi, Bluetooth, GPU and other hardware", "sudo apt install linux-firmware"),
                    ("nvidia-driver", distro_id, "NVIDIA proprietary driver package family", "sudo ubuntu-drivers autoinstall"),
                    ("nvidia-prime", distro_id, "Hybrid graphics switching for NVIDIA Optimus laptops", "sudo apt install nvidia-prime"),
                    ("mesa-utils", distro_id, "OpenGL diagnostic tools such as glxinfo", "sudo apt install mesa-utils"),
                    ("vulkan-tools", distro_id, "Vulkan diagnostic tools such as vulkaninfo", "sudo apt install vulkan-tools"),
                    ("dkms", distro_id, "Dynamic Kernel Module Support", "sudo apt install dkms"),
                    ("build-essential", distro_id, "Build tools for compiling kernel modules and software", "sudo apt install build-essential"),
                    ...
                    ("tlp", distro_id, "Laptop battery optimization tool", "sudo apt install tlp"),
            ...
            # Debian
            packages.extend([
                ("firmware-iwlwifi", debian_id, "Intel Wi-Fi firmware package on Debian", "sudo apt install firmware-iwlwifi"),
                ("firmware-realtek", debian_id, "Realtek firmware package on Debian", "sudo apt install firmware-realtek"),
                ("firmware-misc-nonfree", debian_id, "Various non-free firmware package on Debian", "sudo apt install firmware-misc-nonfree"),
                ("nvidia-driver", debian_id, "NVIDIA proprietary driver package on Debian", "sudo apt install nvidia-driver"),
            ...
            # Fedora
            packages.extend([
                ("linux-firmware", fedora_id, "Firmware package for hardware support", "sudo dnf install linux-firmware"),
                ("akmod-nvidia", fedora_id, "NVIDIA driver package via RPM Fusion", "sudo dnf install akmod-nvidia"),
                ("xorg-x11-drv-nvidia-cuda", fedora_id, "NVIDIA CUDA support package via RPM Fusion", "sudo dnf install xorg-x11-drv-nvidia-cuda"),
            ...
            for distro_id in arch_like_ids:
                packages.extend([
                    ("linux-firmware", distro_id, "Firmware package for hardware support", "sudo pacman -S linux-firmware"),
            ...
            # openSUSE
            packages.extend([
                ("kernel-firmware-all", opensuse_id, "Firmware packages for hardware support", "sudo zypper install kernel-firmware-all"),
                ("nvidia-driver", opensuse_id, "NVIDIA proprietary driver package family", "sudo zypper install-new-recommends"),
            ...
            commands = [
                ...
                (None, "Show recent NVIDIA errors", "dmesg | grep -i nvidia", "low", "Searches kernel log for NVIDIA-related messages", None),
                (None, "Show recent Wi-Fi errors", "dmesg | grep -i wifi", "low", "Searches kernel log for Wi-Fi-related messages", None),
                ...
                (None, "Show system summary", "inxi -Fxxx", "low", "Shows full hardware summary if inxi is installed", None),
            ...
            # Ubuntu / Pop!_OS / Linux Mint commands
            for distro_id in [ubuntu_id, popos_id, mint_id]:
                commands.extend([
            ...
            commands.extend([
                ...
                (debian_id, "Install miscellaneous non-free firmware", "sudo apt install firmware-misc-nonfree", "medium", "Installs various firmware packages", "sudo apt remove firmware-misc-nonfree"),
            ...
            commands.extend([
                ...
                (fedora_id, "Install NVIDIA akmod", "sudo dnf install akmod-nvidia", "medium", "Installs NVIDIA driver package via RPM Fusion", "sudo dnf remove akmod-nvidia"),
                (fedora_id, "Install NVIDIA CUDA support package", "sudo dnf install xorg-x11-drv-nvidia-cuda", "medium", "Installs NVIDIA CUDA support package via RPM Fusion", "sudo dnf remove xorg-x11-drv-nvidia-cuda"),
            ...
            # Arch / Manjaro / EndeavourOS commands
            for distro_id in arch_like_ids:
                commands.extend([
            ...
            commands.extend([
                ...
                (opensuse_id, "Install recommended packages", "sudo zypper install-new-recommends", "medium", "Installs recommended packages, often including drivers", None),
    ...
    print(f"Distros inserted/checked: {len(distros)}")
    print(f"Drivers inserted: {len(drivers)}")
    print(f"Devices inserted: {len(devices)}")
    print(f"Packages inserted: {len(packages)}")
    print(f"Commands inserted: {len(commands)}")
